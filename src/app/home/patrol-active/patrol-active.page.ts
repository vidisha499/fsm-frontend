...
//       });
//     },
//     error: () => this.showToast('Could not find active patrol')
//   });
// }


// patrol-active.page.ts

openObservationModal(type: string) {
  // Loader dikhayein kyunki API call ho rahi hai
  this.http.get(`${this.apiUrl}/active`).subscribe({
    next: (data: any) => {
      // Backend aksar array bhejta hai [ {id: 73, ...} ]
      const activePatrol = Array.isArray(data) ? data[0] : data;
      
      if (activePatrol && activePatrol.id) {
        // ID milte hi aage badhein
        this.navCtrl.navigateForward(['/home/sightings'], {
          queryParams: { 
        ...
        // Agar array khali hai []
        this.showToast('No active patrol found in database. Start trip again.');
      }
    },
    error: (err) => {
      console.error("Fetch Active Error:", err);
      this.showToast('Network Error: Cannot find active session');
    }
  });
}
