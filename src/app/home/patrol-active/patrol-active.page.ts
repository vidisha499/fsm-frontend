...
export class PatrolActivePage implements OnInit, OnDestroy, AfterViewInit {
  ...
  gpsWatchId: any; 
  isSubmitting: boolean = false;
  isSaving: boolean = false; // New animation state
  isFinished: boolean = false; 
  capturedPhoto: string | null = null;
  totalDistanceKm: number = 0;
  lastLatLng: L.LatLng | null = null;
  ...
  constructor(private navCtrl: NavController, private alertCtrl: AlertController, private loadingCtrl: LoadingController, private http: HttpClient) {}
  ...
  async saveObservation() {
    ...
    this.http.get(`${this.apiUrl}/active`).subscribe((activeData: any) => {
      ...
      // API Call to save in 'sightings' table
      this.http.post(`${this.apiUrl}/sightings`, payload).subscribe({
        ...
        error: () => {
          this.isSaving = false;
          alert("Error saving log to database");
        }
      });
    });
  }
