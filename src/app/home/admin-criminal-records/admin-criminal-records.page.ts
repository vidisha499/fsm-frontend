...
@Component({
  selector: 'app-admin-criminal-records',
  templateUrl: './admin-criminal-records.page.html',
  styleUrls: ['./admin-criminal-records.page.scss'],
  standalone: false
})
export class AdminCriminalRecordsPage implements OnInit {
  ...
  /**
   * Match a report against V2 hierarchy filter names.
  ...
  loadHierarchy() {
    ...
    this.dataService.getHierarchyForFilters(companyId.toString()).subscribe({
      next: (h) => {
        ...
        if (this.allReportsCache) {
          this.applyFiltersToCache(this.filterFrom, this.filterTo);
        }
      },
